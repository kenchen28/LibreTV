...
app.get('/api/music/:action', async (req, res) => {
  try {
    ...
    if (action === 'detail') {
      ...
      // Extract cover image from <img id="record-img" src="...">
      const coverMatch = html.match(/id="record-img"[^>]*\bsrc="([^"]+)"/);
      const cover = coverMatch ? coverMatch[1] : '';

      // Extract LRC lyrics from <div class="lrc">...<section>...<article...>...<br> lines </article></section></div>
      // xymp3 puts lyrics inside an article with <br> separators
      let lrc = '';
      ...
      if (urlMatch) {
        return res.json({ code: 200, data: { mp3Url: urlMatch[1], cover, lrc } });
      }
