...
try {
  ...
  if (agentsJsonString) {
    ...
    if (Array.isArray(parsedAgents) && parsedAgents.length > 0) {
      ...
      console.log(
        `[代理日志] 已从环境变量加载 ${USER_AGENTS.length} 个 User Agent。`,
      );
    } else {
      console.warn(
        "[代理日志] 环境变量 USER_AGENTS_JSON 不是有效的非空数组，使用默认值。",
      );
    }
  } else {
    console.log(
      "[代理日志] 未设置环境变量 USER_AGENTS_JSON，使用默认 User Agent。",
    );
  }
} catch (e) {
  // 如果 JSON 解析失败，记录错误并使用默认值
  console.error(
    `[代理日志] 解析环境变量 USER_AGENTS_JSON 出错: ${e.message}。使用默认 User Agent。`,
  );
}
...
function resolveUrl(baseUrl, relativeUrl) {
  ...
  try {
    ...
    logDebug(
      `URL 解析失败: base="${baseUrl}", relative="${relativeUrl}". 错误: ${e.message}`,
    );
    // 简单的备用逻辑
...
async function fetchContentWithType(targetUrl, requestHeaders) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logDebug(
        `请求失败: ${response.status} ${response.statusText} - ${targetUrl}`,
      );
      // 创建一个包含状态码的错误对象
      const err = new Error(
    ...
    logDebug(
      `请求成功: ${targetUrl}, Content-Type: ${contentType}, 内容长度: ${content.length}`,
    );
    // 返回结果
...
async function processMasterPlaylist(url, content, recursionDepth) {
  ...
  if (recursionDepth > MAX_RECURSION) {
    throw new Error(
      `处理主播放列表时，递归深度超过最大限制 (${MAX_RECURSION}): ${url}`,
    );
  }
  const baseUrl = getBaseUrl(url);
  const lines = content.split("\n");
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].startsWith("#EXT-X-STREAM-INF")) {
      ...
      const currentBandwidth = bandwidthMatch
  ...
  if (!bestVariantUrl) {
    logDebug(
      `在主播放列表 ${url} 中未找到有效的子列表 URI，将其作为媒体列表处理。`,
    );
  ...
  const { content: variantContent, contentType: variantContentType } =
    await fetchContentWithType(bestVariantUrl, {});

  // 检查获取的内容是否是 M3U8
  if (!isM3u8Content(variantContent, variantContentType)) {
    logDebug(
      `获取的子播放列表 ${bestVariantUrl} 不是 M3U8 (类型: ${variantContentType})，将其作为媒体列表处理。`,
    );
    return processMediaPlaylist(bestVariantUrl, variantContent);
  }

  // 递归处理获取到的子 M3U8 内容
  return await processM3u8Content(
    bestVariantUrl,
    variantContent,
    recursionDepth + 1,
  );
...
async function validateAuth(req) {
  ...
  const serverPasswordHash = crypto
    .createHash("sha256")
  ...
  try {
    ...
    if (pathData) {
      if (Array.isArray(pathData)) {
        ...
        console.info(
          `从 req.query["...path"] (数组) 组合的编码路径: ${encodedUrlPath}`,
        );
      } else if (typeof pathData === "string") {
        encodedUrlPath = pathData; // 也处理 Vercel 可能只返回字符串的情况
        console.info(
          `从 req.query["...path"] (字符串) 获取的编码路径: ${encodedUrlPath}`,
        );
      } else {
        console.warn(
          `[代理警告] req.query["...path"] 类型未知: ${typeof pathData}`,
        );
      }
    } else {
    ...
    if (!targetUrl) {
      // 抛出包含更多上下文的错误
      throw new Error(
        `无效的代理请求路径。无法从组合路径 "${encodedUrlPath}" 中提取有效的目标 URL。`,
      );
    }
    ...
    // --- 获取并处理目标内容 ---
    const { content, contentType, responseHeaders } =
      await fetchContentWithType(targetUrl, req.headers);
    ...
    console.error(
      `[代理错误处理 V3] 捕获错误！目标: ${targetUrl || "解析失败"} | 错误类型: ${error.constructor.name} | 错误消息: ${error.message}`,
    );
    ...
    if (
      ...
      console.error(
        "请再次检查函数代码及所有辅助函数中，是否有 const 声明的变量被重新赋值。",
      );
